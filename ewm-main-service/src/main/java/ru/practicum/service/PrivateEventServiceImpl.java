...
public class PrivateEventServiceImpl implements PrivateEventService {
    ...
    public EventFullDto createEvent(Long userId, NewEventDto request) {
        ...
        Location location = locationRepository.save(Location.builder().lat(request.getLocation().getLat()).lon(request.getLocation().getLon()).build());
        Event event = eventRepository.save(eventMapper.toEvent(request, user, category, location));
    ...
    public List<EventShortDto> getEvents(Long userId, int from, int size) {
        ...
        return eventHelper.makeShortDtoList(events);
    }
    ...
    public List<ParticipationRequestDto> getEventRequests(Long userId, Long eventId) {
        ...
        return requestRepository.findAllByEventId(eventId).stream().map(requestMapper::toDto).collect(Collectors.toList());
    }
    ...
    public EventRequestStatusUpdateResult updateRequestStatus(Long userId, Long eventId, EventRequestStatusUpdateRequest request) {
        Event event = eventRepository.findById(eventId).orElseThrow(() -> new NotFoundException("Event not found"));
        if (!event.getInitiator().getId().equals(userId)) throw new ConflictException("Not the initiator");

        List<ParticipationRequest> requests = requestRepository.findAllById(request.getRequestIds());
        for (ParticipationRequest req : requests) {
            if (req.getStatus() != RequestStatus.PENDING) throw new ConflictException("Only pending requests can be updated");
        }
        ...
        if (request.getStatus() == RequestStatus.REJECTED) {
            ...
            result.setRejectedRequests(requests.stream().map(requestMapper::toDto).collect(Collectors.toList()));
        } else {
            int confirmedCount = requestRepository.countByEventIdAndStatus(eventId, RequestStatus.CONFIRMED);
            int limit = event.getParticipantLimit();
            if (limit != 0 && confirmedCount >= limit) throw new ConflictException("Limit reached");
